...
const save_status = function(qid)
{
    ...
    let time = timer.count();
    let pt = parseInt($("#point").text(), 10);

    let date = new Date();
    date.setTime(date.getTime() + 30 * 24 * 60 * 60 * 1000);
    let expired = ";expires=" + date.toGMTString();

    let val = qid + "<>" + pt + "<>" + time + "<>" + undone.join(",")
        + "<>" + $("#g_log").val();
    let cookie = "tempopreserve=" + encodeURIComponent(val);
    document.cookie = cookie + expired;
};

const load_status = function(direct_run)
{
    let is_valid = false;
    let res = document.cookie.split(";").reduce((res,param) => {
        let p = param.split("=");
        if (p[0].trim().indexOf('done') == 0) {
            let qid = parseInt(p[0].split("_").pop());
            if (!qid) return res;
            $("#menu .qoption").eq(qid - 1).addClass('cleared');
            return res;
        }
        if (p[0].trim() != "tempopreserve") return res;
        let data = decodeURIComponent(p[1]).split("<>");

        if (data.length < 5) return res;
        res.qid = data.shift();
        res.pt = data.shift();
    ...
    const runresume = () => {
        ...
        res.undone.forEach(kid => $(".kidx" + kid).addClass("undone").show().next().hide());
        //全パーツが開いたグリフを表示
        $(".glyph").each(function() {
...
const load_quiz = function(quiz)
{
    ...
    // パズル描画
    let qlen = draw_puzzle(qlist.split("/"), $("#quiz"),
    ...
    $(document).unbind().on('click', function(e) {
        ...
        $("#keyinput").hide();
        //glyph_expander(30, $(".word").show());
    });

    // カウントアップタイマのイベントハンドラ
    timer.set_handler(count => {
        $('#time').text(count);
        let bpm = qlen * 60 / (count);
    ...
    $("#quiz .elm").unbind().click(function() {
        const glyph_expander = (percent, $word) => {
            let h = (percent * 60 / 100) + "px";
    ...
    $(document).keydown(function(e) {
        ...
        if (e.keyCode == 39 || e.keyCode == 37) {
            light = 0;
            let max = $("#quiz .glyph").size();
            for(let i = 0; i < max; i++) {
                let n = $("#quiz .glyph").index($(".glyph.selected"));
                if (e.keyCode == 39){ n++; }
                if (e.keyCode == 37){ n--; }
                n = (n + max) % max;
...
{
    let files = [$("#gasapi").prop("href"), "qlist.json"];
...
const show_menu = function()
{
    ...
    $("#newest").text("");
    let $option = $('<div>').addClass("qoption").appendTo("#newest");
    let $qid = $('<div>').addClass("qid").appendTo($option).text("00");
...
const show_ending = function()
{
    if ($("#srvlog").size()) return;

    timer.stop();
    $(document).unbind();
    let qid = parseInt($(".qid:last").text()) - 1;

    let bpm = parseInt($("#qlen").text()) * 60 / timer.count();
    let pt = parseInt($("#point").text(), 10);
    ...
    let uname = document.cookie.split(";").reduce((uname, param) => {
        if (uname) return uname;
        let p = param.split("=");
        if (p[0].trim() == "uname") return decodeURIComponent(p[1]);
        return uname;
    });
